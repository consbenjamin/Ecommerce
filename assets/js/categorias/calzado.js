...
function cargarProductos(data) {
  ...
  // Buscamos la categoría "calzado"
  const categoriacalzado = data.find(categoria => categoria.categoria === 'calzado');

  // Verificamos si la categoría fue encontrada
  if (!categoriacalzado) {
    console.error('No se encontró la categoría "calzado" en el archivo JSON');
    return;
  }

  // Verificamos si tiene productos
  if (!Array.isArray(categoriacalzado.productos) || categoriacalzado.productos.length === 0) {
    console.error('No hay productos en la categoría "calzado"');
    return;
  }

  // Generamos las cards de los productos
  const cardsHTML = categoriacalzado.productos.map(producto => {
    return `
      <div class="col-md-4">
