...
function cargarProductos(data) {
  ...
  const categoriaRemeras = data.find(categoria => categoria.categoria === 'remeras');

  // Verificamos si la categoría fue encontrada
  if (!categoriaRemeras) {
    console.error('No se encontró la categoría "remeras" en el archivo JSON');
    return;
  }

  // Verificamos si tiene productos
  if (!Array.isArray(categoriaRemeras.productos) || categoriaRemeras.productos.length === 0) {
    console.error('No hay productos en la categoría "remeras"');
    return;
  }

  // Generamos las cards de los productos
  const cardsHTML = categoriaRemeras.productos.map(producto => {
    return `
      <div class="col-md-4">
